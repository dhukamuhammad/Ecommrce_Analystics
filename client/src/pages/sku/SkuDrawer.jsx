...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend, Area, AreaChart, BarChart, Bar, Brush } from 'recharts';
import api from '../../services/api';

const SkuDrawer = ({ isOpen, onClose, data, formatCurrency }) => {
    // Ab sirf 2 tabs bachenge
                ...
                if (response.data.success) {
                    const formattedSales = response.data.data.salesData.map(item => {
                        ...
                        if (groupBy === 'week') {
                            ...
                            const endDisplayTime = endDateObj.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
                            displayTime = `${displayTime} - ${endDisplayTime}`;
                        }
                        return {
                            ...item,
                            displayTime,
    ...
    return (
        <div className="fixed inset-0 z-[100] flex justify-center items-center bg-black/60 backdrop-blur-sm p-4 transition-opacity">
            ...
            {/* NAYA: Width 850px se badhakar 1000px kar di gayi hai (Bada Popup) */}
            <div className="w-[1000px] max-w-[95vw] bg-[#F8FAFC] max-h-[90vh] rounded-xl shadow-2xl flex flex-col overflow-hidden">
                ...
                {/* Tabs Section - Ab sirf 2 Tab bachenge */}
                ...
                <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-6 custom-scrollbar">
                    ...
                    {/* --- TAB 1: QUANTITY, ORDERS & FINANCIALS (Ek Sath, Chhote Boxes) --- */}
                    {activeTab === 'qty_finance' && (
                        <div className="flex flex-col gap-5">

                            {/* NAYA: Order Metrics Ab Yahi Aa Gaya */}
                            <div>
                                <h4 className="text-[13px] font-bold text-[#243463] uppercase tracking-wider mb-3 flex items-center gap-2">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Total Orders</span>
                                        <span className="text-[18px] font-bold text-[#243463]">{data?.Total_Orders || 0}</span>
                                    </div>
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm border-b-4 border-b-green-500">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Organic Orders</span>
                                        <span className="text-[18px] font-bold text-green-600">{data?.Organic_Orders || 0}</span>
                                    </div>
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm border-b-4 border-b-teal-500">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Ad Orders</span>
                                        <span className="text-[18px] font-bold text-teal-600">{data?.Ad_Orders || 0}</span>
                                    </div>
                                </div>
                            </div>

                            {/* Logistics Breakdown (Chhote Boxes) */}
                            <div>
                                <h4 className="text-[13px] font-bold text-[#243463] uppercase tracking-wider mb-3 flex items-center gap-2">
                                ...
                                <div className="grid grid-cols-5 gap-3">
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Total Qty</span>
                                        <span className="text-[18px] font-bold text-[#243463]">{data?.Total_Quantity || 0}</span>
                                    </div>
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm border-l-4 border-l-blue-500">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Shipment</span>
                                        <span className="text-[18px] font-bold text-blue-600">{data?.Shipment_Quantity || 0}</span>
                                    </div>
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm border-l-4 border-l-red-500">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Refund/Return</span>
                                        <span className="text-[18px] font-bold text-red-600">{data?.Refund_Quantity || 0}</span>
                                    </div>
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm border-l-4 border-l-orange-500">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Cancelled</span>
                                        <span className="text-[18px] font-bold text-orange-600">{data?.Cancel_Quantity || 0}</span>
                                    </div>
                                    <div className="bg-white p-3 border border-[#E5E7EB] rounded-lg shadow-sm border-l-4 border-l-[#243463]">
                                        <span className="text-[11px] text-[#6B7280] font-medium block mb-1">Replacement</span>
                                        <span className="text-[18px] font-bold text-[#243463]">{data?.Replacement_Quantity || 0}</span>
                                    </div>
                                </div>
                            </div>

                            {/* Financials (Horizontal Grid insted of Stack) */}
                            <div>
                                <h4 className="text-[13px] font-bold text-[#243463] uppercase tracking-wider mb-3 flex items-center gap-2">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-white p-4 border border-[#E5E7EB] rounded-lg shadow-sm">
                                        <span className="text-[12px] text-[#6B7280] font-medium block mb-1">Total Sales</span>
                                    ...
                                    <div className="bg-white p-4 border border-[#E5E7EB] rounded-lg shadow-sm">
                                        <span className="text-[12px] text-[#6B7280] font-medium block mb-1">Settlement</span>
                                    ...
                                    <div className="bg-red-50 p-4 border border-red-100 rounded-lg shadow-sm">
                                        <span className="text-[12px] text-red-600 font-bold block mb-1">Ad Spend</span>
                    ...
                    {activeTab === 'trends' && (
                        <div className="flex flex-col h-full">
                            ...
                            {/* NAYA: Filters aur Custom Date ek hi line me (flex-row) */}
                            <div className="flex items-center gap-2 mb-4">
                                {['1M', '3M', '6M', '1Y', 'CUSTOM'].map(f => (
                                    <button
                                        key={f}
                                        onClick={() => handleTimeFilterChange(f)}
                                        className={`px-3 py-1.5 text-[12px] font-bold rounded-lg transition-all border shadow-sm ${timeFilter === f ? 'bg-[#243463] text-white border-[#243463]' : 'bg-[#F8FAFC] text-[#6B7280] border-[#E5E7EB] hover:bg-gray-100'}`}
                                    >
                                        {f}
                                    </button>
                                ))}

                                {/* Custom Date Inputs with Clear Button */}
                                {timeFilter === 'CUSTOM' && (
                                    <div className="flex items-center gap-2 bg-white px-2 py-1 rounded-lg border border-[#E5E7EB] shadow-sm ml-2">
                                        <input
                                            type="date"
                                            className="text-[12px] p-1 border-none outline-none bg-transparent text-[#243463] cursor-pointer"
                                        ...
                                        {/* NAYA: Custom Date Clear Button */}
                                        <button
                                            onClick={() => {
                                                setCustomDates({ start: '', end: '' });
                                                handleTimeFilterChange('1M'); // Automatically reset to 1 Month
                                            }}
                                            className="ml-1 p-1 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded transition-colors"
                            ...
                            {/* AD SPEND AND STORAGE FEE SIDE-BY-SIDE */}
